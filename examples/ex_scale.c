...
int main(void)
{
   ...
   bmp1 = al_load_bitmap("data/mysha.pcx");
   if (!bmp1) {
      abort_example("Unable to load image\n");
      return 1;
   }

   bmp2 = al_load_bitmap("data/mysha.pcx");
   if (!bmp2) {
      abort_example("Unable to load image\n");
      return 1;
   }

   bmp_w = al_get_bitmap_width(bmp1);
   bmp_h = al_get_bitmap_height(bmp1);

   if(!(timer = al_create_timer(1.0 / frame_rate)))	
   {
	   abort_example("Error create timer\n");
	   return -1;
   }
   ...
   while (true) {
	   al_wait_for_event(queue, &event);
		...
		if (redraw && al_is_event_queue_empty(queue)) 
		{
			...
			al_draw_tinted_scaled_bitmap(bmp1, tint,
				0, 0, bmp_w, bmp_h,
				0, 0,
				bmp_w*cos(theta) , bmp_h,
				flags);
